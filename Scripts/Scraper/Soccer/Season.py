...
class Season(Basic):
    def __init__(self, key, url, info, logger: Logger = None, db: DB = None, path: str = None):
        # initialize
    ...
    def parse_general_info(self):
        self.logger.log(f'Cmd: parse_general_info', level=10)
    ...
    def add_fixture(self, url: str):
        temp = MatchReport(url, logger=self.logger)  # create fixture + scrape
    ...
    def scrape(self):
        ...
        navbar_options = self.navbar()  # get inner navbar links
        for key, url in navbar_options.items():
